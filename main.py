...
with open(file_path, 'r') as file:
    ...
    if not json_data['configLock']:
        ...
        print("----------远程推送----------")
        pushtoken = input("请输入pushplus推送密钥，不需要请留空：")
...
def thisTime(hour,minute):
    ...
    # 获取当天的日期部分，构造一个新的时间结构体，用于设置当天的指定时间
    ...
    # 计算时间差（单位为秒）
    remaining_seconds = int(target_time_stamp - current_time_stamp)
    # 计算剩余小时数
    ...
    print("\r当前时间：{}，距离下次任务执行{}:{} 还剩{}小时{}分钟{}秒".format(
        current_time, target_hour, target_minute, remaining_hours, remaining_minutes, remaining_seconds), end="")
    # time.sleep(1)
...
def job():
    ...
    print("\r进入检索，当前时间为:", current_time)

    errorCookie = qiandao(Cookies)
